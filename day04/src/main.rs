...
fn main() {
    ...
    for (idx, line) in lines.iter().enumerate() {
        ...
        pnums.extend(numbers[1].split_whitespace()
            .filter_map(|num_str| num_str.parse::<i32>().ok()));
        println!("Game {}: Winning Numbers: {:?}, Picked Numbers: {:?}", idx, wnums, pnums);
        println!("Score: {}: ", check_winners(&wnums, &pnums));
        println!("---------------------------");
    }
...
fn check_winners(winning_numbers: &Vec<i32>, picked_numbers: &Vec<i32>) -> i32{
    ...
    println!("Numbers that Hit: {:?}", common_numbers);

    common_numbers.len() as i32 * 2
}